...
/**
 *
 * Title         : Aqua Resizer - S3 Compatible
 * Description   : Based on https://github.com/syamilmj/Aqua-Resizer -- but compatible with Amazon S3 
 * Version       : 1.0
 * Author        : Richard Yu
 * Author URI    : richardyu.org
 * License       : MIT - https://opensource.org/licenses/MIT
 * Documentation : https://github.com/richardyu314/aqua-resizer-s3
 * Dependencies  : You must install the AWS PHP SDK from https://aws.amazon.com/sdk-for-php
 *
 */
    ...
    class Aq_Resize
    {
        ...
        public function process( $url, $width = null, $height = null, $crop = null, $single = true, $upscale = false ) {
            try {
                ...
                $s3url = ""; // the S3 or CDN url that you want to serve images from, no trailing slash
                ...
                if ( ! $dims && ( ( ( null === $height && $orig_w == $width ) xor ( null === $width && $orig_h == $height ) ) xor ( $height == $orig_h && $width == $orig_w ) ) ) {
                    ...
                    elseif ( file_exists( $destfilename ) && getimagesize( $destfilename ) ) {
                        $img_url = "$s3url/wp-content/uploads{$dst_rel_path}-{$suffix}.{$ext}";
                    ...
                    // Else, we resize the image and return the new resized image url.
                        ...
                        if ( ! is_wp_error( $resized_file ) ) {	
                            ...
                            $img_url = "$s3url/wp-content/uploads" . $resized_rel_path;
							try {							
								$bucket = ""; // your S3 bucket name
								$keyname = $resized_rel_path;
								
								$s3 = new S3Client([
									'version' => 'latest',
									'region'  => '', // your AWS region, such as us-west-2
									  'credentials' => array(
										'key' => '', // your AWS IAM Key
										'secret'  => '' // your AWS IAM Secret
									  )
								]);
								...
								$result = $s3->putObject([
									'Bucket' => $bucket,
									'Key'    => 'wp-content/uploads'.$keyname,
									'SourceFile'   => '/var/www/html/wp-content/uploads'.$resized_rel_path,
									'ACL'    => 'public-read'
								]);
							} catch (S3Exception $e) {
